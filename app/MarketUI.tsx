...
export function SegmentedView<T extends string>({value,onChange,options,label="View"}:{value:T;onChange:(value:T)=>void;options:ViewOption<T>[];label?:string}){const selected=Math.max(0,options.findIndex(option=>option.key===value));return <div className="view-toggle" role="group" aria-label={label} style={{"--selected-index":selected,"--view-count":options.length} as CSSProperties}><i className="view-slider" aria-hidden="true"/>{options.map(option=><button key={option.key} className={value===option.key?"active":""} aria-pressed={value===option.key} onClick={()=>onChange(option.key)}><span aria-hidden="true">{option.icon}</span><b>{option.label}</b></button>)}</div>}
