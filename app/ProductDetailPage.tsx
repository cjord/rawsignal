...
import type {CatalogDetail,DetailPeerContext,DetailPeerQuartiles,DetailPriceVariant,EbayAskHistory,EbayListingSample,EbayListingSnapshot,GradedCardData,PeerAnchorStats,PriceHistory,RiftboundPairMetrics,SealedDetail,SignalStrictness} from "../core/domain/types";
...
const signedMultiple=(value:number)=>`${value>=0?"+":"−"}${Math.abs(value).toFixed(2)}×`;
const signedPercent=(value:number)=>`${value>=0?"+":"−"}${Math.abs(value).toFixed(0)}%`;

function RiftboundPairPanel({pair,setName}:{pair:RiftboundPairMetrics;setName:string}){
 const averagePct=pair.averageMultiplier?pair.differenceFromAverage/pair.averageMultiplier*100:0,setPct=pair.setAverageMultiplier?pair.differenceFromSetAverage/pair.setAverageMultiplier*100:0;
 return <section className="detail-section detail-riftbound-pair"><header><span>Riftbound comparison</span><h2>Signature / Overnumbered Pair</h2></header><div className="detail-history-grid">
  <Metric label="Individual multiplier" value={multiple(pair.multiplier)} hint={`Signature ${formatUsd(pair.signatureMarketPrice)} ÷ Overnumbered ${formatUsd(pair.overnumberedMarketPrice)}`}/>
  <Metric label="All-pair average" value={multiple(pair.averageMultiplier)} hint={`${signedMultiple(pair.differenceFromAverage)} (${signedPercent(averagePct)}) vs average · ${pair.pairCount} pairs`}/>
  <Metric label="Set average" value={multiple(pair.setAverageMultiplier)} hint={`${signedMultiple(pair.differenceFromSetAverage)} (${signedPercent(setPct)}) vs ${setName} average · ${pair.setPairCount} pairs`}/>
 </div><p className="detail-note">Compares the TCGplayer Market price of this matched Signature and Overnumbered pair. Exact set, card name, and collector number must match; ambiguous pairs are excluded.</p></section>;
}
...
export default function ProductDetailPage({detail,market,serverTiming}:{detail:CatalogDetail;market?:string;serverTiming?:string|null}){
 ...
 {detail.kind==="single"&&detail.riftboundPair&&<RiftboundPairPanel pair={detail.riftboundPair} setName={detail.set}/>}
 ...
 {detail.kind==="sealed"&&<><PackProfilePanel profile={detail.packProfile} evidence={detail.pullRateEvidence} breakdown={detail.valueBreakdown} setName={detail.set}/><PullRatesSection detail={detail}/></>}
