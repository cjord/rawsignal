...
import type {CatalogDetail,DetailPriceVariant,GradedCardData,PeerAnchorStats,PriceHistory,SealedDetail,SignalStrictness} from "./domain/types";
...
function PeerLine({label,count,average,current,noun}:{label:string;count:number;average:number|null;current:number|null;noun:string}){
 ...
 return <p className="detail-peer-context">Average {label}: <b>{formatUsd(average,"N/A")}</b> across {count.toLocaleString()} others{delta!=null&&<> · this {noun} sits <b className={delta<0?"down":"up"}>{formatPercent(Math.abs(delta)).replace("+","")} {delta<0?"below":"above"}</b> that average</>}.</p>;
}
...
function FairValuePanel({history,current,midPrice,kind,peerAnchor}:{history:PriceHistory|null;current:number|null;midPrice:number|null;kind:"single"|"sealed";peerAnchor:PeerAnchorStats|null}){
 ...
 return <section className="detail-section detail-fair-value">
  ...
  <p className="detail-note">{anchor!=null
  ...
  } An informational model — not a valuation guarantee or financial advice.</p>
 </section>;
}
...
function SignalsPanel({history,current,strictness}:{history:PriceHistory|null;current:number|null;strictness:SignalStrictness}){
 return <section className="detail-section"><header><span>Signal check</span><h2>Hot Buy / Hot Sell</h2></header>{history?<div className="detail-signals">{(["buy","sell"] as const).map(side=>{const evaluation=evaluateMarketSignal(history.points,side,strictness,current);return <div key={side} className={`detail-signal-card${evaluation.eligible?` is-${side}`:""}`}><small>{side==="buy"?"Hot Buy":"Hot Sell"}</small>{evaluation.eligible?<><DetailSignalBadge signal={evaluation.signal}/><p>{evaluation.signal.detail}</p></>:<p className="detail-signal-miss"><b>Not qualifying.</b> {evaluation.detail}</p>}</div>})}</div>:<p className="detail-unavailable">Signal evaluation needs price history.</p>}<p className="detail-note">Checked at {strictness} strictness — adjustable in display settings. Signals are informational qualification checks against this printing&apos;s history. They are not guarantees or financial advice.</p></section>;
}
...
export default function ProductDetailPage({detail,market}:{detail:CatalogDetail;market?:string}){
 ...
 <SignalsPanel history={historyData} current={current} strictness={strictness}/>
