...
import {ebaySearchUrl} from "../core/domain/marketplace-links";
...
function EbayMarketPanel({detail,current}:{detail:CatalogDetail;current:number|null}){
 const item={kind:detail.kind,name:detail.name,set:detail.set,number:detail.kind==="single"?detail.number:null};
 ...
 const links=<div className="detail-actions ebay-actions"><a className="tcgplayer-button ebay-button" href={searchHref} target="_blank" rel="noopener noreferrer">Browse eBay listings ↗</a><a className="tcgplayer-button ebay-button" href={soldHref} target="_blank" rel="noopener noreferrer">eBay sold listings ↗</a></div>;
 ...
 return <section className="detail-section detail-ebay"><header><span>Marketplace</span><h2>eBay Listings</h2></header>
  ...
  {ebay!=null&&ebay.samples.length>0&&<div className="detail-table-scroll"><table className="detail-variants-table ebay-listings"><thead><tr><th scope="col">Listing</th><th scope="col">Ask</th><th scope="col">Shipping</th><th scope="col">Condition</th></tr></thead><tbody>
   {ebay.samples.map(sample=><tr key={sample.itemId}><th scope="row"><a href={sample.url} target="_blank" rel="noopener noreferrer">{sample.title}</a></th><td>{formatUsd(sample.price)}</td><td>{sample.shipping==null?"—":sample.shipping===0?"Free":formatUsd(sample.shipping)}</td><td>{sample.condition??"—"}</td></tr>)}
  </tbody></table></div>}
  {links}
