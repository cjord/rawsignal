...
export default function DeferredImage({src,alt,className=""}:Props){
 ...
 return <span ref={host} className={`deferred-image ${loaded?"is-loaded":""} ${!src?"is-empty":""} ${className}`}>{src&&near&&idle&&<img src={src} alt={alt} loading="lazy" decoding="async" fetchPriority="low" onLoad={()=>setLoaded(true)}/>}</span>;
}
