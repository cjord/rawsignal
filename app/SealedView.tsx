...
export default function SealedView(){
 ...
 return <section className="sealed-market" id="sealed-market">
  ...
  <div className="sealed-rows">{loading?<div className="empty">Loading sealed products…</div>:visible.map(product=>{const result=calculate(product);return <a className="sealed-row" href={product.url} target="_blank" rel="noreferrer" key={product.productId}><span className="sealed-product"><DeferredImage src={product.image} alt=""/><span><b>{product.name}</b><small>{product.set}</small></span></span><span className="sealed-category">{product.category}</span><span data-label="MSRP"><b>{usd(product.msrp)}</b><small>{taxOn?`Cost ${usd(result.cost)}`:product.msrpSource}</small></span><span data-label={basis==="market"?"Market":"Median"}><b>{usd(result.value)}</b><small>{keepPct<100?`Proceeds ${usd(result.proceeds)}`:"TCGplayer value"}</small></span><span data-label="Profit" className={result.profit>=0?"profit-positive":"profit-negative"}><b>{result.profit>=0?"+":""}{usd(result.profit)}</b></span><span data-label="Profit %"><b className={`profit-pill ${result.profitPct>=0?"positive":"negative"}`}>{result.profitPct>=0?"+":""}{result.profitPct.toFixed(1)}%</b></span></a>})}</div>
