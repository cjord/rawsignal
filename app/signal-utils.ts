...
export function marketSignal(points:PricePoint[],side:"buy"|"sell",strictness:SignalStrictness,currentOverride?:number|null):MarketSignal|null{
 const sorted=[...points].filter(p=>p.price>0).sort((a,b)=>a.date.localeCompare(b.date));
 const current=currentOverride??sorted.at(-1)?.price;if(!current||!Number.isFinite(current))return null;
 ...
 if(!candidates.length)return null;const best=candidates.sort((a,b)=>a.distance-b.distance)[0],opposite=side==="buy"?Math.max(...best.prices):Math.min(...best.prices),swing=side==="buy"?(opposite-current)/opposite*100:(current-opposite)/opposite*100;
 ...
 if(best.distance>cutoff||score<preset.minScore)return null;
 ...
 return{side,score,confidence,reason,detail:`${pct(swing)} ${side==="buy"?"below":"above"} the opposite ${period} extreme · ${pct(cutoff)} ${strictness} cutoff`,distance:best.distance,cutoff};
}
