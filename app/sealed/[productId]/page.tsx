...
export default async function SealedDetailRoute({params,searchParams}:Props){const [route,query]=await Promise.all([params,searchParams]),detail=await detailRecord("sealed",route.productId,query.market);if(!detail||detail.kind!=="sealed")notFound();return <ProductDetailPage detail={detail} market={query.market} serverTiming={detailServerTiming(detail)}/>}
