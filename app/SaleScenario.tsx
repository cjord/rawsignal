...
export default function SaleScenario({keepPct,onKeepPct,taxOn,onTaxOn,taxRate,onTaxRate,shipping,onShipping,profitableOnly,onProfitableOnly,showProfitability=true}:Props){
 const active=Number(keepPct!==100)+Number(taxOn)+Number(shipping>0)+Number(showProfitability&&profitableOnly);
 const selected=[taxOn?"tax":"",showProfitability&&profitableOnly?"profitable":""].filter(Boolean) as string[];
 return <section className={`sale-scenario ${active?"has-scenario":""}`} aria-labelledby="sale-scenario-title">
  ...
  <div className="sale-scenario-controls">
   ...
   <CheckboxGrid className="movement-filters scenario-checks" options={[{key:"tax",label:"Include sales tax"},...(showProfitability?[{key:"profitable",label:"Profitable products only"}]:[])]} selected={selected} onToggle={key=>key==="tax"?onTaxOn(!taxOn):onProfitableOnly(!profitableOnly)}/>
  </div>
 </section>;
}
