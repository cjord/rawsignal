...
export default function MetricsView({payload}:{payload:MetricsPayload|null}){
 ...
 const pokemonBase=payload?rebase(series["index:pokemon-cards"]??[],(series["index:riftbound-cards"]??[])[0]?.date??""):[];
 const riftboundBase=payload?rebase(series["index:riftbound-cards"]??[],(series["index:riftbound-cards"]??[])[0]?.date??""):[];
 return <main className="detail-page metrics-page"><TopBar className="detail-topbar" active="metrics" strictness={strictness} onStrictness={changeStrictness}/>
  <article className="detail-content">
   ...
   <section className="detail-section"><header><span>Tracked market</span><h2>Overview</h2></header>
    ...
    <p className="detail-note">Tracked value sums current market prices across every tracked product — coverage, not capitalization. Movement follows each market&apos;s equal-weighted index series.</p></section>
