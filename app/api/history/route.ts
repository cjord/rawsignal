...
export async function GET(request: Request) {
  ...
  const englishNm = (payload.result ?? []).filter(row => row.language === "English" && row.condition === "Near Mint");
  const exact = englishNm.find(row => row.variant.toLowerCase() === printing.toLowerCase());
  const selected = exact ?? englishNm[0];
  ...
  return NextResponse.json({ points, variant: selected.variant, condition: "Near Mint", coverage: exact ? "exact" : "fallback" }, {
