...
export function createMemoryCatalogRepository(cards: Card[], sealedProducts: SealedProduct[], enrichments:CatalogDetailEnrichment[]=[]): CatalogRepository {
  ...
  return {
    ...
    async getDetail(kind,productId,market){
      ...
      const packs=uniqueSealed.filter(item=>item.set===product.set&&item.category==="Booster Packs"&&item.marketPrice!=null&&item.marketPrice>0),packPrice=packs.length?Math.min(...packs.map(item=>item.marketPrice as number)):null;
