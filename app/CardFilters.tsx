...
export default function CardFilters({sets,selectedSets,onSets,minPrice,maxPrice,onMinPrice,onMaxPrice,movement,onMovement,onReset}:Props){
 ...
 return <details ref={root} className={`card-filters ${active?"has-filters":""}`}><FilterButton active={active} description="Price, sets & movement"/><div className="filter-panel">
  ...
  <SearchableCheckboxGrid legend="Available sets" options={sets.map(set=>({key:set,label:set}))} selected={selectedSets} onChange={onSets} searchLabel="Search card sets"/>
