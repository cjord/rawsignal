...
export default function CardFilters({sets,selectedSets,onSets,minPrice,maxPrice,onMinPrice,onMaxPrice,movement,onMovement,onReset}:Props){
 ...
 return <details ref={root} className={`card-filters ${active?"has-filters":""}`}><summary><span aria-hidden="true">⌁</span><b>Filters</b>{active>0&&<em>{active}</em>}<small>{active?"Active filters":"Price, sets & movement"}</small><i className="filter-chevron" aria-hidden="true">⌄</i></summary><div className="filter-panel">
