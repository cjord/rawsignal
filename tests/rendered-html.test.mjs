...
test("keeps core controls and chart interactions accessible", async () => {
  ...
  const perPageSelect = await readFile(new URL("../app/PerPageSelect.tsx", import.meta.url), "utf8");
  assert.match(perPageSelect, /aria-label=\{label\}/);
  assert.match(page, /label="Cards per page"/);
  assert.match(page, /"Singles market"/);
  assert.match(page, /"Sealed market"/);
  assert.match(sealed, /label="Sealed products per page"/);
  ...
  // The shared tile list (HistoryPanel.standardHistoryMetrics) carries the 90-day row
  // for every card-shaped popover; the page wires it through cardHistoryMetrics below.
  assert.match(await readFile(new URL("../app/HistoryPanel.tsx", import.meta.url), "utf8"), /movementMetric\("90 day",\s*history\?\.change90/);
  assert.match(page, /standardHistoryMetrics\(card\.marketPrice,\s*card\.midPrice,\s*history\)/);
