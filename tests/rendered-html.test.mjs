...
test("normalizes rarity and set multi-selection",async()=>{const [page,sealed,multi,css]=await Promise.all([readFile(new URL("../app/page.tsx",import.meta.url),"utf8"),readFile(new URL("../app/SealedView.tsx",import.meta.url),"utf8"),readFile(new URL("../app/MultiSelectField.tsx",import.meta.url),"utf8"),readFile(new URL("../app/market-views.css",import.meta.url),"utf8")]);assert.match(page,/pokemon:\["illustration-rares","special-illustration-rares"\]/);assert.match(page,/riftbound:\["overnumbered"\]/);assert.match(page,/searchable=\{false\}/);assert.match(page,/setSelectedSets\(\[\]\)/);assert.match(sealed,/setGame\(event\.target\.value as Game\);setSelectedSets\(\[\]\)/);assert.match(multi,/next\.length===options\.length\?\[\]:next/);assert.match(multi,/searchable&&<label className="multi-search"/);assert.match(css,/\.multi-options label:has\(input:checked\),\.set-filters label:has\(input:checked\)/)});
