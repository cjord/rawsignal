...
test("riftbound sealed categories are the shared canonical product-type buckets", async () => {
  const sealed = parseSealedProducts(await json("../public/data/sealed-riftbound.json"));
  ...
  // Decision D3: the feed carries canonical strings directly — no alias vocabulary.
  ...
  assert.ok(decks.allItems.every(product => product.category === "Starter / Theme Decks"));
});
