...
3. **Crawlers over the detail long tail.** 2.7 k views a day across 18 k product URLs defeats a ten-minute page cache; the isolate caches cover the whole-game rows and the set list, but ~5 k rows of set-scoped reads run per view. *2026-09-04: the page cache is now keyed by the publish signature and held for 36 h, so each URL renders once per colo per publish; `robots.txt` and `/sitemap.xml` steer polite crawlers to the set pages.* Cloudflare analytics that day showed ~3.8 requests and ~30 KB per visit with 4xx up 907%: mostly automated. Remaining levers are the operator's dashboard controls (Bot Fight Mode and Block AI bots, a managed challenge on the detail paths for non-verified datacenter traffic, a rate limit, Turnstile on the import form) and, in code, a precomputed similar-items table.
