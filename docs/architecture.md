...
- Rows and artwork are non-navigational; marketplace actions are explicit buttons — the TCGplayer and eBay tiles under the artwork of every history popover (`HistoryPopover` `links`; the stats grid stays market data only) and the TCGplayer, PriceCharting, and eBay buttons on the detail page (`core/domain/marketplace-links.ts` builds every outbound URL; TCGplayer links are the Impact affiliate tracking link deep-linking to the product page, marked rel=sponsored; eBay links are rewritten at click time by the EPN Smart Links script the root layout loads, with the popover off; both footers carry the disclosure).
- eBay figures are active-listing asks from the Browse API (`ebay_listings`), never sales; they are labelled as asks and never enter modeled fair value or signals. The lazy detail grid includes listing images and affiliate item URLs, discloses the sample size and fetch time, and never displays an expired snapshot. PokemonPriceTracker remains the separate source for completed-sale data.
