...
- eBay figures are active-listing asks from the Browse API (`ebay_listings`), never sales; they are labelled as asks and never enter modeled fair value or signals. The lazy detail grid includes listing images and affiliate item URLs, discloses the sample size and fetch time, and never displays an expired snapshot. The Browse request sends the EPN campaign/reference header so eBay returns `itemAffiliateWebUrl`; if that optional field is absent, Raw Signal adds the same direct campaign/reference parameters to `itemWebUrl`. PokemonPriceTracker remains the separate source for completed-sale data.
