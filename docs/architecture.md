...
- Per-isolate memos front the reads every detail view repeats: the early-value sibling-set list and the peer-anchor cohort summary (, todo Q9) are kept ten minutes per database, since both change only when a run publishes.
