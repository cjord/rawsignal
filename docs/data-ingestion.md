...
npm run data:sync:singles -- --riftbound-only
...
The optional `--riftbound-only` refresh regenerates only Riftbound singles and its
scoped manifest. It preserves Pokémon feeds and the overall index freshness dates,
recording the Riftbound refresh separately under `marketFreshness.riftbound`.
Eligible Riftbound singles may have null market prices: the live walk persists their
identities without synthetic history and clears obsolete signals. On-demand eBay
lookups remain available without a market-price bracket; ordinary English-language
and exact promo-treatment checks still apply. Supplemental foreign-exclusive singles
are appended as a small checkpointed work entry, with no new upstream catalog requests.
Both ingestion paths use the same inclusion policy; rejected reason counts document
tokens, basic runes, ordinary promos and foreign duplicates separately. The bundled
manifest records the scoped source population, not a worldwide foreign-card census.
...
2026-09-17: corrected pack aggregates use `pack-v2:` tier keys and ordinary-printing
prices. Readers exclude legacy highest-printing rows; incomplete price tiers have no EV.
The next normal live walk populates corrected rows after deploy. Physical composition
and set-specific odds are generated by `npm run data:build:packs`; see
`docs/pack-profiles-2026-09.md`. The original implementation description below is historical.
