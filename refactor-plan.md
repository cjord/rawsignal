...
- Milestone 11 completed locally on 2026-08-25: added deterministic fallback-coverage and signal-exclusion tests, URL-level critical journeys, and a focused Playwright Chromium suite. The Node 24 quality workflow installs Chromium, and `npm run check` now requires the production build, 73 Node tests, lint, and three browser journeys.
- Milestone 12 completed locally on 2026-08-25: replaced starter documentation, documented architecture/data ownership/legacy artifacts, removed unused starter database and package-manager artifacts, normalized project commands, and added maintainer-document contract coverage.
- Refactor validation gate passed on 2026-08-25: the production build, lint, migrations, ingestion/backfill tests, shared-query parity, rendered contracts, 73 Node tests, and three Chromium journeys pass. The observed Illustration Rare reduction was traced to the bounded transitional history fallback rather than query drift.

### Post-refactor Cloudflare program

The refactor program above is complete. Direct Cloudflare hosting is a separate future operations program, not an unfinished refactor milestone. OpenAI Sites remains the development, testing, and production publishing target. An isolated Cloudflare staging Worker and migrated D1 database prove build/binding compatibility, protected checkpointed ingestion, exact history persistence, and safe feed fallback. Full catalog/history backfills, database parity, Workflows, Cron, monitoring, production D1, routes, and hostname cutover remain intentionally incomplete. Deploy or mutate direct Cloudflare resources only when the user explicitly requests Cloudflare work.
...
Added deterministic fallback-coverage tests, explicit signal exclusion evaluations, and a critical-journey smoke test that restores and mutates a Singles URL, executes shared filtering/sorting, switches to Sealed, and verifies the corresponding product-type result. A focused Playwright Chromium suite now proves Back/Forward restoration, Singles/Sealed and view switching, responsive phone controls, filter dismissal, and persisted display preferences against a suite-managed local server. The suite also caught and fixed missing accessible names on icon-only mobile segmented controls and horizontal overflow in mobile Medium/Text rows. `npm run check` requires the production build, 73 Node tests, lint, and all three browser journeys; the Node 24 GitHub Actions workflow installs Chromium and applies the same gate.
