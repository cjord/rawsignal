...
export type LiveFeedTarget = { kind: "sections"; sections: string[] } | { kind: "sealed"; market: string };
...
export async function handleLiveFeed(request: Request, env: StagingJobEnv): Promise<Response | null> {
  ...
  try {
    if (!await publishedIngestion(env.DB)) return null;
    ...
    console.error(JSON.stringify({ event: "live_feed_failed", path: new URL(request.url).pathname, message: error instanceof Error ? error.message : "Unknown failure" }));
    return null;
  }
}
