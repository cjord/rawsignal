...
- The paid-plan continuation is the staging guard Cron (live). Landed slices: live TCGCSV fetch (`db/live-ingestion.ts` — the probe timestamp is the snapshot identity; each publish ingests exactly once) and graded rotation (`db/graded-ingestion.ts` — 90 credits/day against PokemonPriceTracker, key in the `POKEMONPRICETRACKER_API_KEY` Worker secret; do not also run the local graded sync the same day). Remaining: peer accumulation — implement or deploy only after a specific user request.
