...
- `db/`: `schema.ts` (types only; migrations are hand-written, see Data rules), `repository.ts` (statement builders and the idempotent read/write boundary), `readiness.ts`, the ingestion modules (`daily-ingestion`, `live-ingestion`, `detail-ingestion`, `graded-ingestion`, `metrics-ingestion`, `benchmark-ingestion`, `ebay-ingestion`, `rarity-stats`, `history-backfill`, `history-targets`, `ingestion-batch`), `tick-lease.ts` (the guard cron's 170 s single-flight `cron-lease` row in `refresh_state`), `catalog-repository.ts` and `history-read.ts` (the D1 read side), the page services (`metrics-service`, `sets-service`, `early-value`, `peer-anchors` — memoized per isolate for ten minutes), and `run-id.ts` for the `prefix:date` ingestion run-id format.
- `worker/`: `index.ts` (fetch + cron entry), `scheduled-ingestion.ts` and `scheduled-decision.ts` (the guard-cron tick and its pure policy: live → details → graded → metrics → history → ebay), `staging-jobs.ts` (the ops adapter), `live-feeds.ts` (D1-backed `/data/*` feeds with bundled fallback), `edge-cache.ts` and `page-signature.ts` (the colo cache for API/data responses and for pages, keyed by the publish signature of the five published run ids plus the deploy version).
...
- `sync-tcgcsv.mjs`, `sync-sealed.mjs`, `sync-sealed-onepiece.mjs`, `scripts/`: feed generation — Singles and `tcg-index.json` from TCGCSV; normalized Pokémon sealed; validated One Piece sealed (category 68); `scripts/validate/` and `scripts/io/` for validation manifests and last-good publishing; `scripts/sets/` (TCGdex set index and set logos, written to `app/data/`); `scripts/details/` (detail-feed generator, `npm run data:build:details`); `scripts/graded/sync-graded.mjs` (budgeted PokemonPriceTracker sync); `scripts/scalper/` (the Scalper allowlist pipeline, reviewed per `docs/scalper-variant-review.md`); `scripts/cloudflare/` (deploy-config preparation and catalog parity); `scripts/backtest/` (walk-forward harness); `scripts/local-db/` (local max-profile D1 build/swap).
...
| `/cards/[productId]`, `/sealed/[productId]` | `ProductDetailPage` via `detail-route` + `data/load-detail` | Shared detail surface; `detail-tables.tsx` renders the sealed page's Chase cards and same-set tables — the selection rule (set cards priced above the set's cheapest plain booster-pack market price, top by value when no pack price exists, capped at twelve) lives in `core/catalog-repository.ts` |
| `/api/catalog`, `/api/catalog/detail`, `/api/history`, `/api/history/batch`, `/api/signals`, `/api/metrics`, `/api/set-ev`, `/api/collectr` | `app/api/**/route.ts` | Read D1 first, bundled feeds as fallback; the detail pages load on the server instead of calling `/api/catalog/detail` |
...
| Cron | none | none | `*/1` guard cron: claim the 170 s `cron-lease` or exit idle, then one checkpointed batch of the first due job, only when due |
| `ENVIRONMENT` | — | `staging` (enables the ops adapter) | `production` (adapter refuses) |
| Secrets | none | `STAGING_JOB_TOKEN` | job token + `POKEMONPRICETRACKER_API_KEY`; optional `ALPHAVANTAGE_API_KEY` (S&P benchmark on the metrics tick, skips when absent); `EBAY_CLIENT_ID` + `EBAY_CLIENT_SECRET` and var `EBAY_EPN_CAMPAIGN_ID` for the eBay Browse rotation (keyset not yet issued, so the `ebay` action never fires) — each external spender exists only on production |
