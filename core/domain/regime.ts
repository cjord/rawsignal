...
// The price-only evidence every reading is judged on, computed once per series.
type RegimeFeatures={current:number;change7:number|null;change30:number|null;momentum:number|null;drawdown:number|null;jump:number;gapDays:number;robustHigh:number;spread:number;cooling:boolean};

function regimeFeatures(sorted:PricePoint[],window90:number[],current:number,demand:RegimeDemand|null|undefined):RegimeFeatures{
 ...
 return {current,change7,change30,momentum,drawdown,jump,gapDays,robustHigh,spread,cooling:demand!=null&&demand.change<=REGIME_THRESHOLDS.demandCooling};
}

// Spike: one outsized jump between adjacent observations no more than 3 days apart.
function spikeReading(f:RegimeFeatures):RegimeReading|null{
 if(f.jump>=REGIME_THRESHOLDS.spikeDayPct&&f.gapDays<=3)return{regime:"spike",detail:`Jumped ${pct(f.jump)} in a single step — one print is not a trend; confidence is limited until it holds.`};
 return null;
}

// At the 90-day high the question is whether momentum is still building (Breakout),
// fading (Overextended), or mixed (Steady). Returns null away from the high.
function nearHighReading(f:RegimeFeatures,breadth:number|null|undefined,demand:RegimeDemand|null|undefined):RegimeReading|null{
 const t=REGIME_THRESHOLDS,{change7,momentum,cooling}=f;
 const nearHigh=f.robustHigh>0&&f.spread>=5&&(1-f.current/f.robustHigh)*100<=t.nearHighPct;
...
// Away from the high: a sustained slide is Falling, a turn up off a deep drawdown is
// Improving, and everything else is Steady.
function trendReading(f:RegimeFeatures):RegimeReading{
 const t=REGIME_THRESHOLDS,{change7,change30,drawdown}=f;
...
export function classifyRegime(points:PricePoint[],currentOverride?:number|null,demand?:RegimeDemand|null,breadth?:number|null):RegimeReading|null{
 ...
 const features=regimeFeatures(sorted,window90,current,demand);
 return spikeReading(features)??nearHighReading(features,breadth,demand)??trendReading(features);
}
