...
export function parseEbayListing(item:unknown):EbayListingSample|null{
 ...
 const url=typeof item.itemAffiliateWebUrl==="string"?item.itemAffiliateWebUrl:typeof item.itemWebUrl==="string"?item.itemWebUrl:null;
 if(!url)return null;
...
export function summarizeEbayListings(items:unknown[],total:number|null,options:{market?:number|null;sampleCount?:number;minListings?:number}={}):EbayListingSummary{
